...
- Examples: See `examples/core_usage/` (Tier 1) and `examples/handlers_usage/` (Tier 2) for controller and peripheral usage across Arduino, Linux, and PlatformIO platforms
...
**Important:** Revert back to the registry version (`cameronbrooks11/CRUMBS@^0.10.3`) before committing changes, so users get the published stable version.
...
- Examples: `examples/core_usage/` and `examples/handlers_usage/`
