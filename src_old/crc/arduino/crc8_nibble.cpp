 ...
 */
  ...
  #include <pgmspace.h>
#endif
#include "crc8_nibble.hpp" // header file converted by generator
#include <stdlib.h>
#include <stdint.h>
 ...
 */
static const crc8_nibble_t crc8_nibble_crc_table[16] PROGMEM = {
...
crc8_nibble_t crc8_nibble_update(crc8_nibble_t crc, const void *data, size_t data_len)
{
    ...
    while (data_len--) {
        tbl_idx = (crc >> 4) ^ (*d >> 4);
        crc = (pgm_read_byte(crc8_nibble_crc_table + (tbl_idx & 0x0f))) ^ (crc << 4);
        tbl_idx = (crc >> 4) ^ (*d >> 0);
        crc = (pgm_read_byte(crc8_nibble_crc_table + (tbl_idx & 0x0f))) ^ (crc << 4);
        d++;
    }
