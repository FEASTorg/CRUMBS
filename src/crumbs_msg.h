...
{
    if (msg->data_len >= CRUMBS_MAX_PAYLOAD) return -1;
...
{
    if ((size_t)msg->data_len + 2 > CRUMBS_MAX_PAYLOAD) return -1;
...
{
    if ((size_t)msg->data_len + 4 > CRUMBS_MAX_PAYLOAD) return -1;
...
{
    if ((size_t)msg->data_len + sizeof(float) > CRUMBS_MAX_PAYLOAD) return -1;
...
{
    if ((size_t)msg->data_len + len > CRUMBS_MAX_PAYLOAD) return -1;
...
{
    uint8_t tmp;
    if (crumbs_msg_read_u8(data, len, offset, &tmp) != 0) return -1;
    *out = (int8_t)tmp;
    return 0;
}
...
{
    uint16_t tmp;
    if (crumbs_msg_read_u16(data, len, offset, &tmp) != 0) return -1;
    *out = (int16_t)tmp;
    return 0;
}
...
{
    uint32_t tmp;
    if (crumbs_msg_read_u32(data, len, offset, &tmp) != 0) return -1;
    *out = (int32_t)tmp;
    return 0;
}
...
{
    if (offset + sizeof(float) > len) return -1;
